...
local function button(x,y,w,label,active)
 if y>=2 and y+1<term.getSize() then U.button(x,y,w,2,label,active and colors.blue or colors.gray) end
end
function M.run()
 local section=1
 while true do
  local w,h=term.getSize(); local small=(w<70 or h<22)
  U.clear(); U.header('Settings  |  '..sections[section])
  local top=small and 5 or 6
  if w<70 then
  ...
  if section==1 then
   ...
   button(x,top+4,24,'Change Hostname',true)
   button(x,top+(small and 7 or 7),24,'Default: Files',false)
  elseif section==2 then
   ...
   if small then
    button(x,top+7,24,'Toggle Animations',true)
    button(x,top+9,24,'Toggle Sounds',true)
   else
    button(x,top+8,24,'Toggle Animations',true)
    button(x+26,top+8,24,'Toggle Sounds',true)
   end
  ...
  elseif section==5 then
   ...
   button(x,top+4,24,'Toggle Auto Start',true)
   button(x,top+7,24,'Toggle Boot Delay',false)
  elseif section==6 then
   U.label(3,top,'Restore PacificOS preferences to defaults.',colors.yellow)
   button(3,top+3,28,'Reset Settings',true)
  end
  U.button(3,h-2,20,1,'Back',colors.gray)
  U.status('Q / Esc / Backspace = close | Arrows = sections')
  ...
  elseif e=='mouse_click' or e=='monitor_touch' then
   local x0,y=b,c
   local selected=nil
   if w<70 and y>=3 and y<3+#sections*2 and x0>=2 and x0<24 then selected=math.floor((y-3)/2)+1
   elseif w>=70 and y>=3 and y<4 then local tabW=math.max(7,math.floor((w-4)/#sections)); selected=math.floor((x0-2)/tabW)+1 end
   if selected and selected>=1 and selected<=#sections then section=selected
   elseif y>=h-2 and y<h and x0>=3 and x0<23 then return
   elseif section==1 and y>=top+4 and y<top+6 and x0>=x and x0<x+24 then
    term.setCursorPos(x,top+6); term.clearLine(); write('Hostname: '); local s=read(); if s and s~='' then C.set('hostname',s) end
   elseif section==1 and y>=top+7 and y<top+9 and x0>=x and x0<x+24 then C.set('default_app','Files')
   elseif section==2 and small and y>=top+7 and y<top+9 and x0>=x and x0<x+24 then toggle('animations')
   elseif section==2 and small and y>=top+9 and y<top+11 and x0>=x and x0<x+24 then toggle('sounds')
   elseif section==2 and not small and y>=top+8 and y<top+10 then if x0>=x and x0<x+24 then toggle('animations') elseif x0>=x+26 and x0<x+50 then toggle('sounds') end
   ...
   elseif section==6 and y>=top+3 and y<top+5 and x0>=3 and x0<31 then C.reset() end
  end
 end
end
return M
