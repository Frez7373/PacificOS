...
local function get(u)
 local r,e=http.get(u); if not r then return nil,e or 'HTTP failed' end
 local s=r.readAll(); r.close(); return s
end
term.clear(); term.setCursorPos(1,1); print('PACIFICOS 1.4.0 INSTALLER'); print('Repairing/updating system files...'); print('')
...
for i,p in ipairs(files) do
 ...
 local full=ROOT..'/'..p; mkdirs(fs.getDir(full)); local h=fs.open(full,'w')
...
h.close()
print(''); print('PacificOS 1.4.0 installed.'); print('Rebooting...'); os.sleep(1); os.reboot()
