...
for i,p in ipairs(files) do
 write(string.format('[%02d/%02d] %s ',i,#files,p)); local body,e=get(BASE..p)
 if not body then print('FAILED'); print(e); return end
 local full=ROOT..'/'..p; mkdirs(fs.getDir(full)); local h=fs.open(full,'w'); if not h then print('FAILED: cannot write'); return end; h.write(body); h.close(); print('OK')
end
local h=fs.open('/startup.lua','w'); if not h then print('FAILED: cannot write /startup.lua'); return end
...
print(''); print('PacificOS 1.3.0 installed.'); print('Rebooting...'); os.sleep(1); os.reboot()
