...
center(math.max(3,math.floor(h/2)-2),'PacificOS 1.1.0',T.muted)
local steps={'Hardware','System files','Kernel','Services','Graphical interface'}
for i,s in ipairs(steps) do
 local y=math.min(h-3,math.floor(h/2)+i-2); term.setCursorPos(3,y); term.setTextColor(T.muted); write(s..string.rep('.',20-#s))
 term.setTextColor(T.good); write(' OK'); os.sleep(0.18)
end
if not fs.exists(ROOT..'/kernel.lua') then error('System kernel is missing') end
term.setCursorPos(3,math.min(h-1,math.floor(h/2)+#steps)); term.setTextColor(T.accent); write('Starting PacificOS...'); os.sleep(0.35)
